...
from config import ANNOTATION_PATH, DATA_FOLDER, IMAGE_PATH

train = []
with open(DATA_FOLDER + "ImageSets/Main/trainval.txt") as f:
...
test = []
with open(DATA_FOLDER + "ImageSets/Main/test.txt") as f:
    for line in f.readlines():
        if line[-1] == "\n":
            line = line[:-1]
        test.append(line)
...
for i in tqdm.tqdm(train):
    ...
    a = shutil.copyfile(
        ANNOTATION_PATH + i + ".txt", train_path + "labels/" + i + ".txt"
    )
...
for i in tqdm.tqdm(test):
    ...
    a = shutil.copyfile(
        ANNOTATION_PATH + i + ".txt", test_path + "labels/" + i + ".txt"
    )
